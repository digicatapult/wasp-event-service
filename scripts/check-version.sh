...
function check_versions_consistent () {
  ...
  local HELM_VALUES_TAG_VERSION=$(yq eval '.image.tag' ./helm/wasp-event-service/values.yaml)
  local HELM_CHART_VERSION=$(yq eval '.version' ./helm/wasp-event-service/Chart.yaml)
  local HELM_CHART_APP_VERSION=$(yq eval '.appVersion' ./helm/wasp-event-service/Chart.yaml)
